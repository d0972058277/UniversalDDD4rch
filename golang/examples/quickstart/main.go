...
func demonstrateBasicWorkflow(ctx context.Context, service *quickstart.OrderService) {
	...
	// Check final status
	finalOrder := service.GetOrder(ctx, orderID)
...
func demonstrateMaybeUsage(ctx context.Context, service *quickstart.OrderService) {
	...
	if createResult.IsSuccess() {
		...
		orderResult := maybeOrder.ToResult(errorWhenNone)

		if orderResult.IsSuccess() {
			fmt.Printf("Order retrieved via Result: %s\n", orderResult.Value().GetID())
		}
	}
}
...
func demonstrateOperationChaining(ctx context.Context, service *quickstart.OrderService) {
	...
	result := service.CreateOrder(ctx, customerID, amount, "chain-demo").
		...
		Bind(func(orderID string) functional.Result[string] {
			fmt.Printf("Confirmed order %s, now shipping...\n", orderID)
...
func demonstrateDomainEvents(ctx context.Context, service *quickstart.OrderService) {
	...
	eventsResult := service.GetOrderEvents(ctx, orderID)
	if eventsResult.IsSuccess() {
		events := eventsResult.Value()
		fmt.Printf("Events after creation: %d\n", len(events))
		for i, event := range events {
			fmt.Printf("  Event %d: %s at %s\n", i+1, event.GetEventType(), event.GetOccurredAt().Format(time.RFC3339))
		}
	}
	...
	service.ClearOrderEvents(ctx, orderID)
	eventsResult = service.GetOrderEvents(ctx, orderID)
	if eventsResult.IsSuccess() {
		events := eventsResult.Value()
